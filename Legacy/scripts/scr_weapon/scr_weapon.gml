...
function weaponTryFire(owner, origin_x, origin_y, aim_dx, aim_dy)
{
    ...
    if (owner.essence < PLAYER_FIRE_COST) return false; // not enough essence
    ...
    var spawn_layer = INSTANCE_LAYER_NAME;
    ...
    var fire_speed  = PLAYER_FIRE_SPEED;
    if (variable_instance_exists(owner, "fire_speed")) fire_speed = owner.fire_speed;

    var fire_damage = PLAYER_FIRE_DAMAGE;
    if (variable_instance_exists(owner, "fire_damage")) fire_damage = owner.fire_damage;

    var cooldown_steps = PLAYER_FIRE_COOLDOWN;
    if (variable_instance_exists(owner, "fire_cooldown")) cooldown_steps = max(1, owner.fire_cooldown);
    ...
    b.diry      = dir_y;
    b.spd       = fire_speed;
    b.damage    = fire_damage;

    // Reset cooldown & essence
    owner.fire_cd = cooldown_steps;
    playerEssenceSpend(owner, PLAYER_FIRE_COST);
    return true;
}
