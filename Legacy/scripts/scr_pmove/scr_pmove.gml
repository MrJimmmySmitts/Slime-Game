...
// ====================================================================
function pMoveAxis(inst, tilemap_id, axis_dx, axis_dy, inset)
{
    ...
    if (remaining_x != 0) {
        ...
        repeat (abs(floor(remaining_x))) {
            ...
            if (!tilemapSolidTest(inst, tilemap_id, next_x, inst.y, inset)) {
                var prev_x = inst.x;
                inst.x = next_x;
                if (block_enemy) {
        ...
        if (!blocked_x) {
            ...
            if (fract_x != 0)  {
                ...
                if (!tilemapSolidTest(inst, tilemap_id, next_fx, inst.y, inset))  {
                    var prev_fx = inst.x;
                    inst.x = next_fx;
                    if (block_enemy) {
                        var hit_enemy_frac = instance_place(inst.x, inst.y, obj_enemy);
                        if (hit_enemy_frac != noone)  { inst.x = prev_fx; }
                    }
                }
            }
        }
    ...
    if (remaining_y != 0) {
        ...
        repeat (abs(floor(remaining_y))) {
            ...
            if (!tilemapSolidTest(inst, tilemap_id, inst.x, next_y, inset)) {
                var prev_y = inst.y;
                inst.y = next_y;
                if (block_enemy) {
        ...
        if (!blocked_y) {
            ...
            if (fract_y != 0) {
                ...
                if (!tilemapSolidTest(inst, tilemap_id, inst.x, next_fy, inset)) {
                    var prev_fy = inst.y;
                    inst.y = next_fy;
                    if (block_enemy) {
                        var hit_enemy_frac_y = instance_place(inst.x, inst.y, obj_enemy);
                        if (hit_enemy_frac_y != noone) { inst.y = prev_fy; }
                    }
                }
            }
        }
...
function pMoveApply(inst, dx, dy, tilemap_id, inset) {
    pMoveAxis(inst, tilemap_id, dx, 0, inset);
    pMoveAxis(inst, tilemap_id, 0, dy, inset);
}
