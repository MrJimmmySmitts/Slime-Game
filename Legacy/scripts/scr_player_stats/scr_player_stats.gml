...
function playerStatsEnsureBase(_inst)
{
    ...
    _inst.base_hp_max         = variable_instance_exists(_inst, "hp_max")   ? _inst.hp_max   : PLAYER_HEALTH_MAX;
    _inst.base_hp_max  = variable_instance_exists(_inst, "base_hp_max") ? _inst.base_hp_max : _inst.base_hp_max;
    _inst.base_essence_per_container = variable_instance_exists(_inst, "base_essence_per_container") ? _inst.base_essence_per_container : PLAYER_HEALTH_ESSENCE;
    ...
    else _base_capacity = _inst.base_hp_max * _inst.base_essence_per_container;
    _inst.base_essence_max = _base_capacity;

    var _base_from_containers = _inst.base_hp_max * _inst.base_essence_per_container;
    ...
    _inst.base_fire_cooldown  = PLAYER_FIRE_COOLDOWN;
    _inst.base_fire_damage  = PLAYER_FIRE_DAMAGE;
    _inst.base_fire_speed   = PLAYER_FIRE_SPEED;
...
function playerStatsApplyBuff(_stats, _buff, _count)
{
    ...
    if (variable_struct_exists(_buff, "fire_damage")) _stats.fire_damage += _buff.fire_damage * _mult;
    if (variable_struct_exists(_buff, "fire_speed"))  _stats.fire_speed  += _buff.fire_speed  * _mult;
}
...
function playerStatsRecalculate(_inst)
{
    ...
    var _stats = {
        container_max:         _inst.base_hp_max,
        ...
        fire_damage: _inst.base_fire_damage,
        fire_speed:  _inst.base_fire_speed,
    };
    ...
    _stats.fire_damage = max(1, round(_stats.fire_damage));
    _stats.fire_speed  = max(0.25, _stats.fire_speed);
    ...
    _inst.fire_cooldown = _stats.fire_cooldown;

    var _base_damage = _stats.fire_damage;
    _inst.fire_damage = _base_damage;
    playerEssenceApplyDamageBuff(_inst, _base_damage);
    if (variable_instance_exists(_inst, "ability_damage_bonus"))
    {
        _inst.fire_damage = max(1, round(_inst.fire_damage + _inst.ability_damage_bonus));
    }
    _inst.fire_speed        = _stats.fire_speed;
}
