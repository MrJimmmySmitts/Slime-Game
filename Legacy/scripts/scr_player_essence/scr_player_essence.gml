...
function playerEssenceContainerSize(_inst)
{
    ...
    if (!variable_instance_exists(_inst, "PLAYER_HEALTH_ESSENCE")) return PLAYER_HEALTH_ESSENCE;
...
function playerEssenceApplyDamageBuff(_inst, _base_damage)
{
    ...
    var _bonus = playerEssenceActiveContainers(_inst) * PLAYER_HEALTH_DAMAGE_BONUS;
    var _total = max(1, round(_base + _bonus));

    _inst.fire_damage = _total;
}
