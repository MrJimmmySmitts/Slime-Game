// ====================================================================
// Name: scr_inventory.gml
// Author: James Smith
// Description: Player Inventory Functionality
// ====================================================================

// ====================================================================
// Name: invEmpty
// Description: Default state of an empty inventory slot.
// ====================================================================
function invEmpty()  {
...
// ====================================================================
// Name: inventoryBoot
// Description: Builds the inventory subsystem once. 
// Usage: Must be called from gameInit().
// ====================================================================
function inventoryBoot(_slot_count)  {
    
    var slots_count = INV_COLS * INV_ROWS;

    // Create subsystem root if missing
    // Do I need this? if (!variable_global_exists("Inventory")) 
    global.Inventory = {};
...
function inventoryUiBoot(_slot_w, _slot_h) {
    ...
    global.invSlotW = max(1, _slot_w);
    global.invSlotH = max(1, _slot_h);

    // Panel anchoring & offsets (optional; used by your panel/origin helpers)
    global.invPanelAnchor = INV_PANEL_ANCHOR;
    global.invPanelOffX  = 0;
    global.invPanelOffY  = 0;

    // Optional theme defaults (safe no-ops if you don’t use them)
    global.invBgAlpha     = 0.6;
    global.invBgPadding   = 8;
}
// ====================================================================
...
function inventorySkinBoot() {
    ...
    global.invSlotW = sprite_get_width(global.invSprSlot);
    global.invSlotH = sprite_get_height(global.invSprSlot);
}
